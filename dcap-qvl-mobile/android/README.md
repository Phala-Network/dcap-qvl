...
(`network.phala:dcap-qvl-android`) that bundles:

* `libdcap_qvl_mobile.so` for `arm64-v8a`, `armeabi-v7a`, `x86`, `x86_64`
* Kotlin bindings under `network.phala.dcapqvl` (generated by UniFFI)
...
dependencies {
    implementation("network.phala:dcap-qvl-android:0.4.1")
}
```
