...
def run_command(
    cmd: List[str], cwd: str = None, capture_output: bool = True
) -> Tuple[int, str, str]:
...
"""
        ...
        code, stdout, stderr = run_command(
            ["uv", "sync", "--python", version], cwd=str(test_project)
        )
...
def main():
    ...
    print_success(
        f"Successful versions: {', '.join(successful_versions) if successful_versions else 'None'}"
    )
