...
def show_abi3_info():
    ...
    print("\nHow it works:")
    print(
        "  ✅ Build once for Python 3.8 → Works on Python 3.8, 3.9, 3.10, 3.11, 3.12+"
    )
        ...
        if wheels:
            ...
            for wheel in wheels:
                ...
                if len(parts) >= 5:
                    ...
                    print(
                        f"     Python: {python_tag}, ABI: {abi_tag}, Platform: {platform_tag}"
                    )

                    if "abi3" in abi_tag:
                        print(
                            f"     ✅ This is an abi3 wheel - compatible across Python versions!"
                        )
                    else:
        ...
        if module_file and module_file.endswith(".so"):
            ...
            try:
                result = subprocess.run(
                    ["file", module_file], capture_output=True, text=True
                )
            ...
            if "abi3" in module_file:
                ...
                print(
                    "ℹ️  Extension filename doesn't indicate abi3 (but may still use it)"
                )
...
def main():
    ...
    if success:
        ...
        print(
            f"   The module works correctly on Python {sys.version_info.major}.{sys.version_info.minor}"
        )
    else:
